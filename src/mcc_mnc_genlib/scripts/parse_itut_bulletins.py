...
def dl_bull_all(bnum=1111, dbg=False):
    ...
    while byear <= time.gmtime().tm_year:
        if dl_bull(bnum, byear, dbg=dbg):
            print(
                '[+] downloaded PDF bulletin %i from year %i and converted to text'
                % (bnum, byear)
            )
            bnum += 1
        else:
...
RE_MNC_UPD_LIST_ENDALT = re.compile(
    r'Extra-territorial use\*{3,}', re.IGNORECASE
)
...
def parse_mnc_upd_list(
    fn=PATH_PRE + 'T-SP-OB.1258-2022-OAS-PDF-E.txt', dbg=False
):
    with open(fn, encoding='utf-8') as fd:
        txt = fd.read()
        #
...
# SANC: W-XYZ digits
#
RE_SANC = re.compile(r'([0-9]-[0-9]{3})\s{1,}(.*)', re.IGNORECASE)
...
def main():
    parser = argparse.ArgumentParser(
        description='download ITU-T operational bulletins, convert them to text, extract lists of MNC and SPC'
    )
    parser.add_argument(
        '-d',
        action='store_true',
        help='download and convert from pdf to text all ITU-T bulletins (requires pdftotext)',
    )
    parser.add_argument(
        '-b',
        default=1111,
        type=int,
        help='ITU-T bulletin number to start with, default is 1111',
    )
    parser.add_argument(
        '-j',
        action='store_true',
    ...
    if args.j:
        generate_json(
            MNC_1111,
            PATH_RAW + 'itut_mnc_1111.json',
            [URL_LICENSE_ITUT],
            URL_LICENSE_ITUT,
        )
        generate_json(
            MNC_1162,
            PATH_RAW + 'itut_mnc_1162.json',
            [URL_LICENSE_ITUT],
            URL_LICENSE_ITUT,
        )
        generate_json(
    ...
    if args.p:
        generate_python(
            MNC_1111,
            PATH_RAW + 'itut_mnc_1111.py',
            [URL_LICENSE_ITUT],
            URL_LICENSE_ITUT,
        )
        generate_python(
            MNC_1162,
            PATH_RAW + 'itut_mnc_1162.py',
            [URL_LICENSE_ITUT],
            URL_LICENSE_ITUT,
        )
        generate_python(
