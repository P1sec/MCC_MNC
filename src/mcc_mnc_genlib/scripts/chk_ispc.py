    ...
    if ext:
        if cntr in P1_CNTR:
            ...
            if not found and cntr in COUNTRY_SPEC:
                ...
                if 'cc2' in cntr_spec:
                    print_cntr(
                        P1_CC2[cntr_spec['cc2']], ext=ext - 1, indent='  '
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description='provides information related to ISPC (International Signaling Point Code); if no argument is passed, lists all known ISPC'
    )
    parser.add_argument(
        'ISPC',
        nargs='*',
        help='0 or more 3-8-3 formatted or integer ISPC values',
    )
    parser.add_argument(
        '-x',
        action='count',
        help='provides extended information for associated country',
    )
        ...
        for ispc in args.ISPC:
            if '-' not in ispc:
                ...
                try:
                    ...
                    print(
                        'invalid format for ISPC: 3-8-3 or integer format required'
                    )
                    return 0
            else:
                if ispc.count('-') != 2:
